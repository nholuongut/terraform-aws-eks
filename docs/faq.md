...
All the options that can be customized for worker groups are listed in [local.tf](https://github.com/nholuongut/terraform-aws-eks/blob/master/local.tf) under `workers_group_defaults_defaults`.
...
If the ConfigMap is missing or the contents are incorrect then ensure that you have properly configured the kubernetes provider block by referring to [README.md](https://github.com/nholuongut/terraform-aws-eks/#usage-example) and run `terraform apply` again.
...
This can happen if the kubernetes provider has not been configured for use with the cluster. The kubernetes provider will be accessing your default kubernetes cluster which already has the map defined. Read [README.md](https://github.com/nholuongut/terraform-aws-eks/#usage-example) for more details on how to configure the kubernetes provider correctly.

Users upgrading from modules before 8.0.0 will need to import their existing aws-auth ConfigMap in to the terraform state. See 8.0.0's [CHANGELOG](https://github.com/nholuongut/terraform-aws-eks/blob/v8.0.0/CHANGELOG.md#v800---2019-12-11) for more details.
...
You need to configure the kubernetes provider correctly. See [README.md](https://github.com/nholuongut/terraform-aws-eks/#usage-example) for more details.

## How can I stop Terraform from removing the EKS tags from my VPC and subnets?

You need to add the tags to the VPC and subnets yourself. See the [basic example](https://github.com/nholuongut/terraform-aws-eks/tree/master/examples/basic).
...
- the kubernetes provider is correctly configured like in the [Usage Example](https://github.com/nholuongut/terraform-aws-eks/blob/master/README.md#usage-example). Primarily the module's `cluster_id` output is used as input to the `aws_eks_cluster*` data sources.
